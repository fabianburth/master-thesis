...
Part of the problem is already visible in the data consumption example. Common routing frameworks and libraries are not able to handle parse URLs such as \lstinline|/components/github.com/gardener/etcd-druid/| since the slashes in the name will be interpreted as a path separator.\par 
...
The problem becomes even more difficult to deal with when considering corresponding endpoints for \emph{Sources} and \emph{Resources}. Using the identity properties of the OCM, respective endpoints would look like this:
...
To solve this parsing issues, assigning additional \emph{surrogate keys}, thus \emph{UUIDs} or \emph{auto incremented IDs}, as it is done in common use cases was considered. But this would require additional requests to find the surrogate key of the respective entity. Thus, the API would have to support request using query parameters as indicated by the listing below.
...
The user first sends the first request to retrieve the ID of the respective \emph{Source}. Once the ID is acquired, it may be used to navigate further through the API and, for example, retrieve all versions of the \emph{Source} with that ID.\par
Although every API that exposes its operations through individual resource endpoint is called REST API today \cite{RichardsonMaturityModel}, this form of usage is already much closer to the original REST architectural style defined by Roy Fielding. It is derived from the web and promotes hypermedia as the engine of application's state \cite{RESTArchitecturalStyle}. Thus, the original idea is that RESTful architectures and consequently also RESTful APIs may be navigated just as websites. So a user may know host address of the API. Sending a GET request to that host address will then return a list hyperlinks of further resources. For this API this may be \lstinline|/components|, \lstinline|/sources|, ... . Again, following one of those hyperlinks by sending a GET request will return another list of hyperlinks of further resources available under the currently selected resource, in the case of \lstinline|/components| a list of hyperlinks to all available individual \emph{Components}. This is great for APIs that have a potentially huge number of unknown users, as it is very intuitive and requires almost no documentation to navigate through, just as the web. But it is not suitable for this kind of application which requires running specific potentially expensive queries based on already known entities.\\
...
This approach is simple and clean. All properties to specify an entity's identity may be embedded in the request body in a suitable format such as JSON. There would be no issues regarding URL and parsing.\par 
But compared to the REST API approach, this is not intuitive and requires to know the respective name of the query defined by the application as well as the properties required by that type of query and the respective format in which these properties have to be provided.\par
Thus, this approach was not satisfying either.
