...
\begin{figure}[H]
	...
	\caption[Package Versions and Dependencies as Graph]{Package Versions and Dependencies as Graph \source{Own Representation}}
...
The first line in listing \ref{lst:CypherTransitive} matches the node with the \emph{label} \lstinline|PACKAGE_VERSION| and the key-value-pairs \lstinline|Name: "spring-context"| and \lstinline|Version: "1.2.12"| and assigns the resulting nodes, or in this example rather node as there is just a single one that fulfills this pattern, to the variable \lstinline|p1|. The second and third line match all nodes with the \emph{label} \lstinline|PACKAGE_VERSION| that \lstinline|p1| depends on. Thus, the \lstinline|WHERE| uses a path as a predicate. Thereby, \lstinline|DEPENDS_ON| specifies the \emph{label} of the relationship and the part after the asterisk specifies the minimum and maximum amounts of hops between the nodes. In this case, the minimum is 1 because otherwise it would consider itself and as there is no explicit maximum given, it defaults to infinite, to consider all transitive dependencies. The nodes that fulfill this criteria are assigned to \lstinline|p2| and returned, in this case, \emph{java-jdk 1.3.1} and \emph{spring-core 2.1.4}.\par 
...
This section only provides an overview of the application. The following \emph{API} and \emph{Implementation} sections cover further details.\par
...
As explained in the overview, every \emph{service} of the application somehow builds on the schemata which may be created through the \emph{Types Service} and \emph{API}. Thus, the rationale behind this service, as well as the design decisions involved, are discussed first.\par

\subsubsection{Schema}
As the whole complexity of the architecture kind of revolves around the dynamically configurable \emph{schemata}, a common question especially considering the \emph{schema less} database may be, why the application requires such rigid complete schemata?\par
So, the term \emph{schema less} often leads to the misconception, that the data somehow does not need to abide to any schema. Technically, this is true for most databases attributed with the term schema less, like Neo4j. So the database will allow storing any key-value pairs. The problem is, without prior knowledge about the keys, the data cannot be retrieved. And without prior knowledge about the data types, the data may hardly be aggregated. Thus, even with \emph{schema less} databases, \emph{partial schemata} are enforced in most use cases.\par
...
Thus, the \emph{entity type schemata} have to have a \emph{kind}, \emph{version}, \emph{name} and especially an \emph{identity\_schema} and an \emph{attributes\_schema} property. As the name suggests, the \emph{identity\_schema} and the \emph{attributes\_schema} of type object are themselves again \emph{schemata}. The \emph{identity\_schema} thereby defines all the properties comprising the identity of the entity and the \emph{attributes\_schema} specifies all other properties. As this is very abstract and meta level, below listing shows an exemplary JSON representation of a request body of a request to create a \emph{Component Version} schema. 
...
So the \emph{identity} of this \emph{Component Version} entity type is comprised of a \emph{name} property of type string and a maximum length of 255 characters and \emph{version} property also of type string with no further restriction. Both of these properties are required. The only further property in this example is \emph{labels}, which is an array of objects with a required \emph{name} property of type string and a \emph{value} property of any type. Thus, these array may hold arbitrarily nested JSON objects.\par 
The other properties provided in this request, so \emph{kind}, \emph{version} and \emph{name} are not part of the entity types schema. These properties are used for processing logic.\par \emph{kind} describes the \emph{class of entity types}. Based on the data model defined in the section \ref{sec:Data Model}, this is necessary for \emph{Native Package Version} and \emph{Info Snippet} where multiple entity types of the same class of entity types, thus, \emph{kind}, may be created. In order to have uniform schemata but also especially for extensibility reasons, this property is required to be included with all entity type schemata. This way, the application could be easily extended to store multiple component models.\par 
...
For example, the identity of a \emph{Component} is equal to the identity of a \emph{Component Version} without considering the version property. Similarly for all other entities with a corresponding relationship. Therefore, although this unfortunately is and cannot be represented in the OpenAPI schema of the entity types schema, the application enforces all the entity type's \emph{identity\_schemas} but \emph{Info Snippet} to provide a \emph{version} property. The higher aggregation entity types are then created implicitly with the respective identity.\par 
...
The \emph{Consumption API} is fairly simple, once the \emph{Types API \& Services} are understood. It exposes a single endpoint:

\begin{lstlisting}[caption=Consumption API Endpoint, captionpos=b, label=lst:ConsumptionEndpoint]
/datalake
\end{lstlisting}

This endpoints only accepts HTTP PUT requests, as the respective operation performed by the application is merging, or in the SQL context rather called \emph{upsert}, of the respective entities. The request is validated against the \emph{Consumption API Schema} assembled by the \emph{Consumption API Schema Assembly Service}.\par 
As mentioned before, a REST-like API similar to the \emph{Types API} would not add much value here. The API has to consume huge amounts of data provided by other systems. Thus, the API would not make use of the HTTP verbs anyway. Furthermore, multiple endpoints would result in additional unnecessary requests. A single endpoint, on the other hand, makes the implementation of the adapters easier and entities in a request may be bundled more flexibly.\\

When coming from 


\subsection{Query API \& Services}
...
\section{API}
In the data model section, it is mentioned several times that the properties of the entities as well as several entities cannot be known upfront. As a quick revision, the reasons therefore were that the Security and Compliance Data Lake shall be independent of a specific component model and also open to a variety of data sources. This is also specified in requirement R.1. Thus, the data model in figure \ref{fig:DataModel} omitted properties and defined classes of entity types.\par
When the data model is applied to a specific component model and to a specific set of data sources, instances of the classes of entity types are defined. In figure \ref{fig:RefDataModel} illustrating an application of the data model, the respective component model is the \emph{Open Component Model}, \emph{BDBA Package Version} is defined as an instance of \emph{Native Package Version} and \emph{Vulnerability} is defined as an instance of \emph{Info Snippet}. To provide this flexibility and extensibility, the application's data model has to be configurable at runtime.\footnote{Since in Neo4j each node and relationships stores key-value pairs and does not require a schema,  an initial idea may be to loosen the data model, provide the data received from the different data sources in the adapters with a \emph{label} declaring their respective entity type and then forward it to the data lake, where the top level properties are stored. Without going into detail, the problem with such a rather \emph{schema less} approach is that the data quality may be bad and there is no reliable way to merge or even query the data, as the keys of the properties are entirely up to the data sources. Actually, in order to even know the names of properties, the component model or data sources would have to be inspected.}\par 
This configurability is offered through a \emph{Types API}, which allows the application administrator to define and adjust the data model to the specific use case.\par
Besides, the application exposes two further APIs. A \emph{Consumption API} to validate and store the data received from the adapters. And, of course, a \emph{Query API} to access and analyze the data stored in the graph.\par 
The design of these different APIs is explained in detail throughout the following sections. 

\subsection{Types API}
The \emph{Types API} exposes the following HTTP endpoints:

\begin{lstlisting}[caption=Types API Endpoints, captionpos=b, label=lst:TypesAPI]
/types/component
...
/types/package-native-attributes
\end{lstlisting}
Each of these endpoints accepts HTTP POST requests containing an \emph{OpenAPI} schema \cite{OpenAPISpec}.\par
Simple OpenAPI schemata defining objects would not be sufficient to base the data model upon. To further explain these issues, below shows an exemplary schema. 



To be precise, the \lstinline|/types/component| endpoint accepts the schema for the \emph{Component Version}, the \lstinline|/types/resource| endpoint for \emph{Resource Version}, the \lstinline|/types/source| endpoint for \emph{Source Version}, \lstinline|info-snippets| endpoints for all kinds of \emph{Info Snippets}.
