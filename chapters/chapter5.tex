...
\begin{xltabular}{\linewidth}{|l|X|l|}
	...
	\footnotetext{\textit{aggregation} in this context refers to the "whole/part" semantic of the word \cite{UML}. Since resources and sources are comprised of packages, they are both aggregations of packages. On a model level, the same applies for the relationships between packages and vulnerabilities or licenses as well as between entire deployments and the deployed resources.}
	...
	As an example, a user shall be able to query for all vulnerabilities in a specific resource like query on the aggregation level of resources. But a user shall also be able to query for all vulnerabilities in an entire specific deployment, thus querying on the aggregate level of deployments (querying on this level of aggregation enables to answer where Log4j is deployed). & 1\\ % Hier ist Log4j auch groß geschrieben, Absicht? Anstatt "As an example" gibt es auch for example / for instance. Für Therefore gibt es auch mehrere synonyme wie "In consequence / For this reason". Gehe mal die Thesis durch und ersetze ein paar davon. Ich ändere "Therefore" jetzt nicht mehr 😊 Beliebte Konjunktionen sind bei dir auch "Thus/Thereby" - google mal nach Synonymen, da gibt es gute Alternativen, um Wortwiederholungen zu vermeiden. Inhaltlich ist bisher alles gut geschrieben und verständlich - Sehr gut! Gibt 'n Däumchen👍
	\hline
...
Since this shall be a prototypical implementation, there is a strong focus on fulfilling the functional requirements. Thus, no concrete limits regarding performance or scalability such as a maximum response time of 5 seconds or support for up to 1000 concurrent users are set at this point. %Here kann auch ersetzt werden, wie z.B. mit "At this point / In this case"
...
A common approach in this situations are URIs. %Ausgeschrieben Namen ergänzen Uniform Resource Identifiers (URIs) & im Abkürzungsverzeichnis mit aufnehmen
...
The OCM specifies that the access property is variable. Still in section \ref{sec:Development and Deployment Landscape at SAP Gardener} "Development and Deployment Landscape at SAP Gardener", it is explained that images, thus technical artifacts, and component descriptors are copied from the development landscape into other landscapes and that the access property changes in this process. So the development landscape may serve as single source of truth and the respective artifact URLs %URL fehlt auch im Abkürzungsverzeichnis. Da es eine Standardabkürzung ist, würde ich sie nicht extra einführen, aber im Abkürzungsverzeichnis mit aufnehmen.
...
Another option to determine whether two artifacts are technically the same, even though they are located in different repositories, is through calculating a suitable \emph{normalized digest}.\par %Du verwendest oft die -ing form, wo ich sie gar nicht nötig finde. Bei dem Satz hätte ich am Ende eher erwartet: ..., is to calculate a suitable normalized digest. Gibt's dafür einen bestimmten Grund? Die -ing Form benutzt man ja in der Progressiven Form, d.h. beim Verlauf einer Handlung. In der Thesis, und gerade beim Design machst du ja noch gar nichts, weshalb ich da eher die "to ..." Form erwarte. Falls du noch Zeit und Lust hast, kannst du deine Thesis darauf prüfen 😉
...
After all, the most important aspect to point out here is that although it is not intuitive, only having local identifiers for artifacts is still quite functional regarding the goals which shall be achieved by the data model. %Den Satz musste ich erstmal zweimal lesen, bis ich ihn verstanden habe 😄 -> Evtl. umschreiben
...
%"So" am Anfang eines Absatzes ist sinnlos, da du bei einer Folgerung ja keinen Absatz machst, und wenn du einen Absatz machst, keine Folgerung willst. 
To answer the question %"such as which" ist Umgangssprache. Das kannste dir in der Thesis sparen. Dann lieber such as weglassen 
...
\textbf{Write Performance:} The data may be provided by all sorts of data sources. As described in the reference architecture in section \ref{sec:Integration of the Security and Compliance Data Lake into the SAP Gardener Landscape} "Integration of the Security and Compliance Data Lake into the SAP Gardener Landscape", %Würde die Beschreibung weglassen, da der Leser ja dank \ref dorthin springen kann, wenn er die Überschrift wissen kann. Außerdem: Benutze \glqq Integration of the Security and Compliance Data Lake into the SAP Gardener Landscape\grqq ,... für Hochkommata anstatt "", da dies sonst in Latex scheiße aussieht (STRG+F ist dein Freund 😉). Musst halt noch schauen, wie man dafür die Sprache umstellt, sodass englische Hochkommata gesetzt werden
...
\textbf{Distribution:} The database is the central metadata store of a company, used to monitor the application landscape and increase the transparency of the infrastructure. Therefore, the number of concurrent users will not be exceedingly high. Besides, the whole application and thus, also the underlying database is not business critical for a company. So, database distribution to increase scalability in terms of parallel queries or to increase availability and fault tolerance are not a primary concern for the Security and Compliance Data Lake.\\\\ %Für "So, ..." gibt es auch Synonyme oder kann oft auch weggelassen werden.
...
Furthermore, the relations are usually normalized, which leads to a low level of redundancy and reduces the risk for anomalies during data manipulation, thereby enabling the adherence to the ACID criteria \cite{RelationalDatabaseModel}.\\ %ACID evtl. kurz einführen, auch wenn man das wissen sollte + Abkürzungsverzeichnis
...
Figure \ref{fig:B+tree} provides a holistic view onto the search trees and how they are used in database systems with some simplifications and assumptions.\par %"Of course" bitte meiden, da unnötig
...
Data in secondary storage such as a disk cannot be processed directly by the \emph{central processing unit (CPU)}. First, it must be copied into a primary storage such as main memory which is usually \emph{dynamic random access memory (DRAM)}. %CPU und DRAM / RAM fehlen im Abkürzungsverzeichnis 
...
Based on the sample data in figure \ref{fig:PackageVersionsAndDependencies}, this returns \emph{log4j 1.5.2} and \emph{spring-boot 1.15.3}. The queries are tested with a PostgreSQL 15 database. The respective DDL %DDL ist auch nicht im Abkürzungsverzeichnis
...
The data model is based on (n:m)-relationships. Also, the database's primary purpose is to perform analysis about different entities. Thus, "Which Component Versions contain a vulnerable Log4J %Log4J ... Du weißt schon Bescheid 😉
...
A conclusion of this analysis of the suitability of database technologies for a central metadata store will be drawn in the following.\par  %War kein richtiger Satz 
...
The \emph{programming language} used to implement the services comprising the Security and Compliance Data Lake is \emph{Go} \cite{Golang}. Generally, the services could probably be implemented with any common programming language. %Dann nimm doch ABAP 😉
...
This section only provides an overview of the application. The following \emph{API \& Services} sections cover further details.\par
...
As the entities of different entity types still have to be processed differently, the \emph{Consumption MUX} %Wer oder was ist MUX? -> Abkürzungsverzeichnis! 
...
The \emph{OpenAPI Specification (OAS)} is a technical specification defining a standard, programming language-agnostic interface description for HTTP APIs. The specification is maintained by the \emph{OpenAPI Initiative (OAI)} %Ja klar...OAS ist im Abkürzungsverzeichnis, aber OAI nicht, oder?
...
The \emph{openapi} and the \emph{info} fields are both required. %Für Felder, die den Code referenzieren, wäre der Hintergrund wie in \lstinline|/customers| besser geeignet als kursiv.
...
Thus, the entity type schemas have to have a \emph{kind}, \emph{version}, \emph{name} and especially an \emph{identity\_schema} and an \emph{attributes\_schema} property. %Gleiches wie oben, lieber mit Hintergrund anstatt kursiv. Dann sieht man direkt, dass es aus dem JSON kommt
...
\subsection{Consumption API \& Services}
